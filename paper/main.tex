...
\textbf{Conclusions:}
SLC2A8 expression identifies a pan-cancer metabolic subtype selectively
sensitive to AKT inhibitors, independent of tissue lineage. This biomarker
...
\citep{Jones2020}. Despite this clinical progress, current biomarkers for
...
fructose-induced de novo lipogenesis \citep{DeBosch2012}. However, the
...
cluster enriched for RAS-pathway mutations (KRAS, NRAS, HRAS, BRAF, and
PIK3CA hotspot mutations). Stratifying cell lines by SLC2A8 expression
above the pan-cancer median and RAS-pathway mutation status produces
...
$p$=3.77$\times$10$^{-6}$) and afuresertib (an AKT inhibitor,
$p$=5.20$\times$10$^{-6}$). At the mechanism-of-action (MOA) level,
...
Convergence testing (Section~2.7) showed that the lipogenesis
...
(2)~the presence of a hotspot mutation in any RAS-pathway gene (KRAS,
NRAS, HRAS, BRAF, or PIK3CA). Cell lines meeting both criteria were
...
as a negative control (LDHA, HK2, GAPDH, HK1, PDK1). No multiple
testing correction was applied to individual gene tests, as the gene
set was pre-specified and hypothesis-driven rather than genome-wide. Group comparisons (target versus
